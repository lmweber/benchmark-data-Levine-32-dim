...
We have written R code to pre-process and export these benchmark data sets in standard formats, in order to make it easier for researchers from other fields to access them to test clustering algorithms. This repository contains R code for the 32-dimensional benchmark data set, and the companion repository [benchmark-data-Levine-13-dim](https://github.com/lmweber/benchmark-data-Levine-13-dim) contains code for the 13-dimensional benchmark data set.
...
- [prepare_data_Levine_32dim.R](prepare_data_Levine_32dim.R): R script to load, pre-process, and export data files for the 32-dimensional benchmark data set ("Levine_32dim")

- [population_names_Levine_32dim.txt](data/population_names_Levine_32dim.txt): cell population names for each of the 14 manually gated populations

- FCS files in folder [data](data/): exported data files in FCS format
    - [Levine_32dim.fcs](data/Levine_32dim.fcs): main data file (transformed data, with cluster labels, and labels for each individual)
    - [Levine_32dim_notransform.fcs](data/Levine_32dim_notransform.fcs): without arcsinh transform
